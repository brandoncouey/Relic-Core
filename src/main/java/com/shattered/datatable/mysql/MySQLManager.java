...
import org.database.MySQLDatabase.MySQLConnectionStatus;
import org.database.query.command.SQLCommand;
...
public class MySQLManager {
	...
	public MySQLManager connect(String host, String username, String password) {
		...
		try {
			...
			for (Entry<String, MySQLDatabase> entry : DATABASES.entrySet()) {
				 ...
				 */
				if (database.getStatus() == MySQLConnectionStatus.CONNECTED)
					continue;
	...
	public boolean isConnected(String name) {
		return getMySQLDatabases().containsKey(name) && getMySQLDatabases().get(name).getStatus() == MySQLConnectionStatus.CONNECTED;
	}
